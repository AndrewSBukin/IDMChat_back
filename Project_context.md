...
## 12. Взаимодействие с внешней системой ИДМ

Для пользователей прописано меню. Некоторые пункты должны перенаправляться на ИДМ и ее ответ надо передавать клиенту.
Ответ передается один в один, за исключением ежедневного и ежемесячного отчетов, которые надо привести к единому виду для компаний IDM1 и IDM2.
Для таких запросов сделан метод
[AcceptVerbs("GET", "POST", "PUT", "DELETE")]
[Route("{screenKey}/{*action}")]
...
Проблема: при вызове /api/v1/menugateway/club.daily/daily.view.today параметры пришли screenKey = "club.daily", action = "ProxyRequest234". А строка "daily.view.today" вообще потерялась.

---

## 13. Актуальные задачи
...
- После завершения интеграции с ИДМ провести ускорение ИДМ.

Не очень важные или с неоцененной важностью:
